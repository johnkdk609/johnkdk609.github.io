...
비즈니스 로직을 수행한다는 것은 결국에는 '서비스 호출'을 한다는 것이다. 모델과 뷰를 연결한다는 것은, 컨트롤러가 있을 때 내부적으로 서비스를 호출하고 필요하다면 리포지토리를 호출하는 것이다. 이렇게 서비스, 리포지토리를 합쳐서 'Model' 이라 하였다. 이렇게 처리한 데이터가 다시 컨트롤러로 넘어와서, 컨트롤러가 이제 이것을 View에 전달을 해주는 것이다. 

즉, <b>컨트롤러는 모델과 뷰를 연결해주는 연결자</b>이다. <u>Model 과 View 는 직접 소통하지 않는다.</u>

여기까지가 MVC 패턴이라고 하는 것이다.