...
이러한 기능 외에도, 가령 특정 2차원 배열이나, 메인에서 사용하고 있는 특정 변수에 대해 Step Over을 할 때마다 변화하는 과정을 보고 싶은데 Debugger 창에 해당 변수가 없어서 바로 확인이 어렵다면, 해당 변수를 마우스로 드래그한 다음, 클릭을 해둔 채 디버거 창으로 끌어오면 바로 볼 수 있다. 만약 해당 변수를 디버거 창에서 보고 싶지 않다면, 마우스 우클릭을 하고 'Remove Watch'를 클릭하면 된다. (Remove All Watch 를 클릭하면 기본값으로 돌아간다.)
