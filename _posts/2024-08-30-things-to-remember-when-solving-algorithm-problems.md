...
한 객체에 다양한 정보가 들어있을 수 있다. 가령 플레이어 객체들을 관리한다고 했을 때, 튜플을 이용해서 ```(플레이어 번호, 행, 열, 플레이어의 방향, 플레이어의 체력)``` 과 같이 할 수 있다. 이때 만약 플레이어들이 2차원 배열 격자(grid)에도 배치되어 있을 수 있다. 리스트를 수정할 수도 있고, 격자를 수정할 수도 있는데, 어느 한 쪽을 수정하면 다른 쪽도 수정된 것으로 업데이트 해줘야 하는 것이다.

구현 난이도가 올라갈수록 이러한 객체 관리 문제가 많이 등장한다. 

이를 위해서는 <u>단위 테스트(unit test)를 자주 하며 리스트, 그리드 등의 데이터가 매 순간 잘 연동되어 변화하고 있는지 검토해야 한다.</u>

한 가지 사용할 수 있는 팁은, 만약 매 사이클마다 메서드를 실행했을 때 처음부터 끝까지 리스트에서 객체들의 순서가 바뀌지 않는다면, 아얘 메인에 객체들의 길이만큼 0 으로 초기화 해놓은 리스트를 만들어 상태를 관리할 수도 있다는 것이 있다. 가령 N 명의 플레이어가 있는데, 격자에서 벗어난 사람은 더 이상 움직이지 않는다고 해보자. 이럴 때에는 ```out_grid = [0 for _ in range(N)]``` 로 따로 리스트를 만들어 두고 격자에서 벗어난 사람의 인덱스 값은 1로 바꿔두는 것이다. 그리고 앞으로 플레이어들에게 순차적으로 이동시킬 때, ```if out_grid[i] == 1: continue``` 와 같이 할 수 있는 것이다. 이렇게 하면 매번 리스트에서 격자에서 벗어난 사람을 ```pop()``` 하는 방식으로 관리하는 것보다 훨씬 빠르게, 간편하게, 명료하게 접근할 수 있다.
