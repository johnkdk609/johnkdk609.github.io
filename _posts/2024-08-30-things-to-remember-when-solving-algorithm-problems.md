...
그 중 하나는 문제 분석 단계에서 시간 복잡도를 생각해봐야 한다는 것이다.

어떤 문제를 풀 때에는 다양한 접근 방법이 존재할 수 있다. 어떤 접근 방법을 선택하는지는 다양한 요소가 고려될 수 있다. 어떤 자료구조를 선택하는지, 어떤 알고리즘을 선택하는지, 하드 코딩을 할 것인지, 간결한 코드를 짤 것인지 등.. 다양한 선택지가 있다.
