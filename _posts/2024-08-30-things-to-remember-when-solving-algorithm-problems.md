...
이러한 파이참 디버깅의 경우, 특정 메서드가 잘 동작하는지 검토할 때에 매우 용이하다. 프린트 디버깅이 여러 번의 순회에 대해 원하는 결과가 제대로 출력 되는지 일목요연하게 보기 좋다면, <u>파이참 디버깅은 특정 메서드가 문제를 일으킬 때 정확히 어떤 부분에서 문제가 발생하는지 세밀한 점검을 할 때 효과를 발휘하는 것</u>이다.
